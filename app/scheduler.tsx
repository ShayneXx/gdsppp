  ...
  return <main className="app-shell"><section className="workspace">
    ...
    {workspaceView !== "home" && <div id="module-content" className={`split-home view-${workspaceView} ${mediaUploadEnabled ? "" : "media-disabled"}`}>
      ...
      <section className="copy-pane"><div className="pane-heading copy-heading"><div><p className="eyebrow">{workspaceView === "shooting" ? "SHOOT NOTICE" : "TREND RADAR"}</p><h2>{workspaceView === "shooting" ? "拍摄通告" : "音乐榜 · 颜值榜"}</h2><p className="subtitle">{workspaceView === "shooting" ? "排期生成通告，内容可编辑、复制与拖动排序。" : "自动更新颜值与音乐热点，并整理成可直接使用的文案。"}</p></div>{workspaceView === "inspiration" && <div className="live-tools"><span className="live-status"><i />{hotUpdatedAt ? `${hotUpdatedAt.toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })} 已更新` : "实时同步"}</span><button className="outline-btn" onClick={() => fetchHot(false)} disabled={hotLoading}>{hotLoading ? "同步中…" : "↻ 刷新"}</button></div>}</div>
        ...
        {copyView === "reference" && <div className="reference-browser"><form className="reference-search" onSubmit={searchReferenceImages}><div className="reference-kind" role="group" aria-label="参考图类型"><button type="button" className={referenceKind === "clothing" ? "active" : ""} onClick={() => setReferenceKind("clothing")}>服装穿搭</button><button type="button" className={referenceKind === "makeup" ? "active" : ""} onClick={() => setReferenceKind("makeup")}>妆容妆照</button></div><div className="reference-searchbar"><input value={referenceQuery} onChange={event => setReferenceQuery(event.target.value)} placeholder={referenceKind === "clothing" ? "搜索：通勤穿搭、法式裙装、运动造型…" : "搜索：清透妆、复古妆、氛围妆照…"} aria-label="搜索服装或妆容参考图" /><button disabled={referenceLoading}>{referenceLoading ? "搜索中…" : "搜索图片"}</button></div></form>{referenceLoading ? <div className="reference-skeleton">{[1,2,3,4,5,6].map(item => <i key={item} />)}</div> : referenceImages.length ? <div className="reference-grid">{referenceImages.map(image => <article className="reference-card" key={image.id}><a href={image.creditUrl} target="_blank" rel="noopener noreferrer" title="查看图片来源"><img src={image.thumbUrl} alt={image.title} loading="lazy" /></a><div><span>{referenceKind === "clothing" ? "服装参考" : "妆容参考"}</span><p>{image.title}</p><button onClick={() => setReferencePicker(image)}>＋ 加入拍摄通告</button></div></article>)}</div> : <div className="reference-empty"><span>图</span><h3>搜索你的拍摄造型灵感</h3><p>输入风格、颜色或场景，找到图片后可直接加入拍摄通告。</p><button onClick={() => searchReferenceImages()}>浏览推荐图片</button></div>}<p className="reference-license">图片搜索结果来自 Wikimedia Commons，点击图片可查看原始来源与许可信息。</p></div>}
