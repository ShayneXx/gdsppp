...
export function Scheduler() {
  ...
  const [workspaceView, setWorkspaceView] = useState<"shooting" | "inspiration">("shooting");
  const [copyView, setCopyView] = useState<"music" | "beauty" | "auto" | "saved" | "notice">("notice");
  ...
  return <main className="app-shell"><section className="workspace">
    ...
    <nav className="module-switch" aria-label="首页功能分区">
      <button className={workspaceView === "shooting" ? "active" : ""} onClick={() => { setWorkspaceView("shooting"); setCopyView("notice"); }}><span>01</span><div><strong>拍摄工作台</strong><small>拍摄排期 · 拍摄通告</small></div><i>→</i></button>
      <button className={workspaceView === "inspiration" ? "active" : ""} onClick={() => { setWorkspaceView("inspiration"); setCopyView("music"); }}><span>02</span><div><strong>灵感工作台</strong><small>音乐榜 · 颜值榜 · 文案整理</small></div><i>→</i></button>
    </nav>
    <div className={`split-home view-${workspaceView} ${mediaUploadEnabled ? "" : "media-disabled"}`}>
