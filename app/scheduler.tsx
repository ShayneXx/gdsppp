...
type InspirationImage = { id: string; title: string; thumbUrl: string; sourceUrl: string; creditUrl: string };
...
export function Scheduler() {
  ...
  const [socialResolving, setSocialResolving] = useState(false);
  ...
  const resolveSocialImage = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (!socialImageUrl.trim()) return;
    setSocialResolving(true);
    try {
      const response = await fetch("/api/social-image", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: socialImageUrl.trim(), source: referenceSource }) });
      const result = await response.json() as { image?: InspirationImage; error?: string };
      if (!response.ok || !result.image) throw new Error(result.error || "没有找到可用图片");
      setReferencePicker(result.image); setSocialImageUrl("");
    } catch (error) { setToast(error instanceof Error ? error.message : "图片链接解析失败"); }
    finally { setSocialResolving(false); }
  };
  const addReferenceToNotice = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/notices/${taskId}/image?kind=${kind}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sourceUrl: referencePicker.sourceUrl, name: referencePicker.title }) });
      ...
      setReferencePicker(null); setToast(`已添加到通告的${kind === "clothing" ? "服装" : "妆容"}参考`);
