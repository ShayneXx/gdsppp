...
export async function GET() {
  try {
    ...
    const musicFallback = ["热门BGM卡点转场", "情绪副歌氛围片", "甜酷舞蹈卡点", "松弛感清唱片段", "复古旋律变装", "通勤耳机氛围", "情侣对唱片段", "轻快节奏日常", "夜景慢歌情绪", "元气舞蹈跟拍", "治愈系音乐独白", "节拍定格转场"].map((title, index) => ({ rank: index + 1, title, hot: 980000 - index * 43000 }));
    const beautyFallback = ["清透妆容近景", "轻熟通勤穿搭", "自然光氛围写真", "甜酷反差变装", "高级感黑白造型", "夏日清爽妆面", "松弛感居家穿搭", "复古港风妆造", "显气色腮红妆", "简约纯色穿搭", "温柔长发氛围", "镜前快速变装"].map((title, index) => ({ rank: index + 1, title, hot: 960000 - index * 41000 }));
